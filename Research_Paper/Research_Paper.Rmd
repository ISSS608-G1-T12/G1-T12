...
1) To be able to conduct portfolio analysis and correlation analysis amongst multiple stocks.
...
An ARIMA model is characterized by 3 terms: p, d, q. Which are automatically optimised in the auto-ARIMA model
...
After all the work done in the previous 2 sub-modules. Users would have identified a few candidate stocks that he or she would want to invest in, and so this brings up the questions such as "When do I invest? Is now a good time?". It also leads us into our last and final sub-module which is time series forecasting on a stock pick. 

Users will input the selected stock ticker into the side panel. Here users will also need to select a few other things, namely:

1. Data start date (which determines the amount of data ingested for the forecasting model) 
2. Accuracy measure (which will determine which forecasting model results to retain based on how users like to quantify accuracy)
3. Forecasting period (the period ahead in which users would like to forecast between 1 - 4 weeks.) 

In this case we will use the following inputs:
...
After the forecasting model is run in the server, two outputs will be created.
...
Looking at the forecast for AAPL over the next week. The prices are expected to go up and hence it might be good for the users to take up a position sooner rather than later. 
