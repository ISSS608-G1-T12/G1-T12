...
### 2.1 Setting up the enviroment/libraries
...
### 2.3 Data Visualization - Time Series
...
When we present the time series forecasting results, we will only present a unified/aggregated set of forecasts from the ensemble model to avoid confusing the user. In our application, the ensemble model uses the mean of the forecast of the models with the top 3 MAPE measures. The mean of the forecasts will then be shown in the chart as the expected future share price of the stock that the users will use for decision making. The ensemble model approach is used to combine the strengths and eliminate the weaknenss of each model.
...
### 4.1 Sketch of Proposed Visualization

### 4.2 Suggested Interactivity in the Proposed Visualization
