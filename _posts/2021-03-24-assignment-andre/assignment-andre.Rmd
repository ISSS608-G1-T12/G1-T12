...
```{r}
#Load packages
packages = c("tidyquant","tidyverse","ggpubr","ggplot2","dplyr","cluster","formattable","reshape2","magrittr","zoo","quantmod","tibble","ggdendro","knitr","kableExtra","dendextend","plotly","scales")
for (p in packages){
...
We see that Cluster 4 had the highest Return/Volatility ratio, so we proceed to see how this cluster performed in the next year, 2018. Taking a look at Cluster 4, we see the cluster has the following stocks: ADP, ANSS, CTSH, GOOGL, ISRG, PAYX, PYPL, SNPS. 
...
```{r}
df <- data.frame(datasource = c("Year","Variables"), 
